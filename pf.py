...
class Image:
    def __init__(self):
        # capture = cv2.VideoCapture('videos/football_mid.mp4')
        ...
        self.capture = cv.CreateFileCapture('videos/football_mid.mp4')
        #captureのサイズを変更(プレゼン用)
        # cv.SetCaptureProperty(self.capture,cv.CV_CAP_PROP_FRAME_WIDTH,1920)
        # cv.SetCaptureProperty(self.capture,cv.CV_CAP_PROP_FRAME_HEIGHT,1080)
...
def initStateVectors(imageSize,sampleSize):
    xs = [i for i in range(1620, 1650)]
    ys = [i for i in range(320, 385)]
    # vxs = [1 for i in range(1620, 1650)]
    # vys = [0 for i in range(320, 385)]

    # xs = [random.uniform(0,imageSize[0]) for i in range(sampleSize)]
    # ys = [random.uniform(0,imageSize[1]) for i in range(sampleSize)]
    vxs = [random.uniform(0,5) for i in range(1620, 1650)]
    vys = [random.uniform(0,5) for i in range(320, 385)]
